...
stripebotMessage = lambda message: print(f"{colorama.Fore.WHITE}[{colorama.Fore.GREEN}StripeBot{colorama.Fore.WHITE}] {colorama.Fore.CYAN}{message}{colorama.Fore.WHITE}")
...
async def addItem(ctx, name, price):
    ...
    try:
        ...
        await ctx.send(f"Added item {name} with price {price.unit_amount / 100} USD. Item ID: {item.id}, Price ID: {price.id}")
...
async def buy(ctx):
    try:
        embed = discord.Embed(title="StripeBot", description="Please select an item to buy. Once you've selected your item, just send a message with the item's ID.", color=0x00ff00)
        for item in stripe.Product.list():
            if item.active:
        ...
        session = stripe.checkout.Session.create(
            ...
            line_items=[{
                'price': stripe.Price.list(product=itemID).data[0].id,
                'quantity': 1,
            }],
            mode='payment',
        ...
        message = await ctx.send(f"Please pay for your item [here]({session.url}). I will automatically mark your item as paid once you have paid.")

        while session.payment_status != "paid":
            await asyncio.sleep(1)
            session = stripe.checkout.Session.retrieve(session.id)
        embed = discord.Embed(title="StripeBot", description=f"You have paid for ```{item.name}```. Thank you for your purchase!", color=0x00ff00)
